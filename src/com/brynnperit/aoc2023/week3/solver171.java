...
public class solver171 {
    ...
    private static class Edge {
        private int[] lowestPathThrough = { Integer.MAX_VALUE, Integer.MAX_VALUE,
                Integer.MAX_VALUE };

        public int lowestPathThrough(int consecutiveDirections) {
            return lowestPathThrough[consecutiveDirections];
        }

        public void setLowestPathThrough(int consecutiveDirections, int elapsedCost) {
            lowestPathThrough[consecutiveDirections] = elapsedCost;
            for (int index = lowestPathThrough.length - 1; index >= consecutiveDirections; index--) {
    ...
    private static class Path {
        ...
        public List<Path> getNextPaths() {
            ...
            for (Direction direction : edges.keySet()) {
                ...
                boolean goingBackwards = goingTo != null && direction == goingTo.opposite();
                if (sameDirection && consecutiveDirections < 2) {
                ...
                } else if (!sameDirection && !goingBackwards) {
                    if (elapsedCost < edge.lowestPathThrough(0)) {
                        nextPaths.add(new Path(this, direction, 0, city, edge));
                    }
                }
            }
            return nextPaths;
