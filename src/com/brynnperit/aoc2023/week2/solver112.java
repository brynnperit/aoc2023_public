...
public class solver112 {
    private static class GalaxyMap {
        final private static long EXTRA_DISTANCE_MULTIPLIER = 1000000;
        ...
        private long getDistanceBetweenGalaxies(int firstGalaxyIndex, int secondGalaxyIndex) {
            long distance = 0;
            ...
            long rowDistance = galaxyRows.getDistanceBetweenGalaxies(firstGalaxyCoordinate, secondGalaxyCoordinate);
            long columnDistance = galaxyColumns.getDistanceBetweenGalaxies(firstGalaxyCoordinate,
        ...
        private static class GalaxyRowColumnList {
            ...
            public long getDistanceBetweenGalaxies(GalaxyCoordinate first, GalaxyCoordinate second) {
                ...
                return distance - extraDistance + (extraDistance * EXTRA_DISTANCE_MULTIPLIER);
            }
    ...
    public static void main(String[] args) {
        ...
        try (Stream<String> inputLines = Files.lines(new File("inputs/week2/input_11").toPath())) {
            ...
            galaxyDistanceSum = startRanges.flatMap(startIndex -> LongStream.range(startIndex + 1, numberOfGalaxies)
                    .map(endIndex -> map.getDistanceBetweenGalaxies((int)startIndex, (int)endIndex))).sum();
        } catch (IOException e) {
            e.printStackTrace();
        }
